...
// exmple de numero de whats app 554298400985@s.whatsapp.net

app.post('/webhook', async (req, res) => {
    console.log('Requisição recebida:', req.body);
    const numTelefone = req.body.numTelefone + "@s.whatsapp.net";
    const mensagem = req.body.mensagem;

    try {
        await sendMessageWTyping({ text: mensagem }, numTelefone);
...
app.listen(PORT, () => {
						...
						for (const msg of upsert.messages) {
							if (!msg.key.fromMe && doReplies) {
								try {
									...
									const numTelefone = msg.key.remoteJid;
									const mensagemRecebida = msg.message?.conversation; // ou outro campo dependendo do tipo da mensagem
				
									// Enviar dados para o backend
									const response = await axios.post('http://localhost:8080/api/wpp', {
										numTelefone,
									...
									// Enviar resposta do backend ao usuário
									await sendMessageWTyping({ text: mensagem }, numTelefone);
								} catch (error) {
	...
	const sendMessageWTyping = async(msg: AnyMessageContent, jid: string) => {
		if (!sock) {
			throw new Error("Socket não está inicializado");
		}
